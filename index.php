...
<body>
        ...
        $head = [
            ...
            ['title' => 'Відео', 'subtitle' => 'всі відео новини']
			        ...
			        for ($i=0; $i < count($head); $i++) { 
						#echo $head[$i]['title'] . '<br>';
						echo '<h3>' . $head[$i]['title'] . '</h3>';
             			echo '<h4>' . $head[$i]['subtitle'] . '</h4>';

						$query = $db->prepare("SELECT title, url, data FROM news WHERE data = :data AND htitle = :htitle");
	        			$query->execute(array('data' => $data, 'htitle' => $head[$i]['title']));
			            while ($row = $query->fetch())
						{
							$url = $row['url'];
							echo "<p><a href='$url'>" . $row['title'] . $row['data'] . '</a></p>';
	...
	<script type="text/javascript">
		function fync() {
		$("#news-block").empty().load("par.php", function() {
		});
		}; 
	</script>
